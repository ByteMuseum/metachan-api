...
export const searchAnimeQuery = async (params: SearchQueryParams): Promise<AnimeSearchResult> => {
    try {
        ...
        const response = await axios.get<JikanSearchResponse>(`https://api.jikan.moe/v4/anime?${queryString}`);
        ...
        Logger.info(`Found ${results.length} unique results for query: ${params.q}`, { prefix: 'Anime Search', timestamp: true });
        ...
        Logger.error(error instanceof Error ? error : `Search query failed for: ${params.q}`, { prefix: 'Anime Search', timestamp: true });
        throw error;
    }
};
