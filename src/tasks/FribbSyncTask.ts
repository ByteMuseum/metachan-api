  ...
  private async processBatch(mappings: FribbList[]): Promise<void> {
    for (const mapping of mappings) {
      try {
        ...
        const mal_anilist_composite =
          mapping.mal_id && mapping.anilist_id ? `${mapping.mal_id}-${mapping.anilist_id}` : null;
        ...
        let entity = await this.repository.findOne({
          where: [
            ...
            ...(mal_anilist_composite ? [{ mal_anilist_composite: mal_anilist_composite }] : []),
          ],
        });
